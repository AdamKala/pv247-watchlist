...
const MovieDetail = async ({
	...
	if (type !== null) {
		...
		databaseMovie ??= await pullAndStoreFromCSFDId(id, type);
	} else {
...
const MovieDetailWrapper = async ({ searchParams }: PageProps) => {
	...
	if (hasCsfd) {
		if (Number.isNaN(csfdId) || (type !== 'series' && type !== 'movie'))
			return <div>Invalid CSFDId or type</div>;
		return (
			<div>
